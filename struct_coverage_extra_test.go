...
func TestParseStruct_DefaultsValid(t *testing.T) {
    ...
    var c C
    c.TZ = now
    if err := ParseStruct(&c); err != nil { t.Fatalf("unexpected error: %v", err) }
    if c.D.String() != "12.34" || c.BS == 0 || c.I != 42 || c.U64 != 45 || c.SE != "red" || len(c.DS) != 2 {
        t.Fatalf("unexpected parsed defaults: %+v", c)
    }
    if c.TZ != now { t.Fatalf("expected TZ retained, got %v", c.TZ) }
}
...
func TestParseStruct_InvalidDefaults(t *testing.T) {
    cases := []struct {
        ...
        {"time", func() error { ResetForTesting(nil); type S struct{ T time.Time `flag:"t" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default time"},
        {"timeLayout", func() error { ResetForTesting(nil); type S struct{ T time.Time `flag:"t" layout:"2006-01-02" default:"bad-date"` }; var s S; return ParseStruct(&s) }, "invalid default time"},
        {"decimal", func() error { ResetForTesting(nil); type S struct{ D decimal.Decimal `flag:"d" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default decimal"},
        {"ip", func() error { ResetForTesting(nil); type S struct{ IP net.IP `flag:"ip" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default ip"},
        {"ipnet", func() error { ResetForTesting(nil); type S struct{ N net.IPNet `flag:"n" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default cidr"},
        {"url", func() error { ResetForTesting(nil); type S struct{ U neturl.URL `flag:"u" default:"http://[bad"` }; var s S; return ParseStruct(&s) }, "invalid default url"},
        {"uuid", func() error { ResetForTesting(nil); type S struct{ ID uuid.UUID `flag:"id" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default uuid"},
        {"bytesize", func() error { ResetForTesting(nil); type S struct{ B ByteSize `flag:"b" default:"10XB"` }; var s S; return ParseStruct(&s) }, "invalid default bytesize"},
        {"durslice", func() error { ResetForTesting(nil); type S struct{ D []time.Duration `flag:"d" default:"1s,notdur"` }; var s S; return ParseStruct(&s) }, "invalid default duration slice element"},
        {"map", func() error { ResetForTesting(nil); type S struct{ M map[string]string `flag:"m" default:"badentry"` }; var s S; return ParseStruct(&s) }, "invalid default map entry"},
    {"json", func() error { ResetForTesting(nil); type S struct{ J json.RawMessage `flag:"j" default:"{bad"` }; var s S; return ParseStruct(&s) }, "invalid default json"},
        {"regexp", func() error { ResetForTesting(nil); type S struct{ R *regexp.Regexp `flag:"r" default:"["` }; var s S; return ParseStruct(&s) }, "invalid default regexp"},
        {"bool", func() error { ResetForTesting(nil); type S struct{ B bool `flag:"b" default:"notbool"` }; var s S; return ParseStruct(&s) }, "invalid default bool"},
        {"int", func() error { ResetForTesting(nil); type S struct{ I int `flag:"i" default:"NaN"` }; var s S; return ParseStruct(&s) }, "invalid default int"},
        {"int64", func() error { ResetForTesting(nil); type S struct{ I int64 `flag:"i" default:"NaN"` }; var s S; return ParseStruct(&s) }, "invalid default int64"},
        {"duration", func() error { ResetForTesting(nil); type S struct{ D time.Duration `flag:"d" default:"nonsense"` }; var s S; return ParseStruct(&s) }, "invalid default duration"},
        {"uint", func() error { ResetForTesting(nil); type S struct{ U uint `flag:"u" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default uint"},
        {"uint64", func() error { ResetForTesting(nil); type S struct{ U uint64 `flag:"u" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default uint64"},
        {"float", func() error { ResetForTesting(nil); type S struct{ F float64 `flag:"f" default:"bad"` }; var s S; return ParseStruct(&s) }, "invalid default float64"},
    }
    for _, c := range cases {
